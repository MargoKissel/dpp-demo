...
export default async function ProductPage({ params }: { params:{sku:string} }) {
  ...
  const rawImg     = normalizeImageUrl(product.image_url)
  const img        = rawImg ? `/api/image?src=${encodeURIComponent(rawImg)}` : null
      ...
      <article className="max-w-md w-full bg-white rounded-xl shadow overflow-hidden">
        {img && (
          <div className="w-full h-64 bg-gray-100 flex items-center justify-center">
            {/* обычный <img> быстрее, потому что картинка уже оптимизирована прокси */}
